$(function(){
    ...
    $("#item21 option:not(:eq(0))").remove();
    var serviceclass = $("#serviceclass").val();

    var target = ctx +"/dashboard/service/item/"+serviceclass+"/serviceitem";
    $.post(target, function(data){
        var item = $.parseJSON(data);
        var arr = item.itemList;
        for(var i = 0; i < arr.length; i++){
            $("#item21").append("<option value='"+arr[i].id+"'>"+arr[i].itemname+"</option>");
        }
    });
    ...
    $("#item21").change(function(){
        ...
        var target = ctx +"/dashboard/service/item/"+id+"/content";
        $.post(target, function(data){
            var item = $.parseJSON(data);
